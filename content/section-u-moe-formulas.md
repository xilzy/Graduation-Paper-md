...
对 $c\in\lbrace Cb,Cr\rbrace$，先计算两源色度相对中性值 128 的偏离：
...
$\mathrm{Linear}_{16\to16}$。位置 $\delta\in\lbrace 1,\ldots,K^2\rbrace$ 的动态门控和样本特异卷积核为：
...
对尺度路径 $s\in\lbrace 1,2,3\rbrace$，源码递归增加 ACM 深度，再调用同一组四层 Transformer 权重：
...
于是 $H_p=H+p_h,\quad W_p=W+p_w$。170×170 训练特征被填充为 176×176。经 LayerNorm 后，特征划分为：
...
对源 $S\in\lbrace A,B\rbrace$ 与融合图 F，幅值和方向保持度为：
