...
`figs/` 下五组自定义框架/原理图的英文统一使用 Times New Roman；正文和微型标签按正文插图可读性重新放大，最小字号统一为 9.5 pt，普通说明约 9.5–11 pt。分布式机制对比图采用五行两列排版，避免十个模块横向挤压。
...
/ytech_m2v4_hdd/lizhongyin/venv/gifnet/bin/python script/make_distributed_parallelism_figure.py
```
